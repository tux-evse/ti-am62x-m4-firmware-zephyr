...
MEMORY
    {
    FLASH (rx) : ORIGIN = 0x0, LENGTH = (0 * 1024 - 0x0 - 0x0)
    RAM (wx) : ORIGIN = 0x0, LENGTH = (192 * 1K)
    DDR : ORIGIN = (2629828608), LENGTH = (4096)
...
.intList :
{
 ...
 KEEP(*(.gnu.attributes))
    ...
    DDR (NOLOAD) : { __DDR_start = .; KEEP(*(DDR)) KEEP(*(DDR.*)) __DDR_end = .; } > DDR __DDR_size = __DDR_end - __DDR_start; __DDR_load_start = LOADADDR(DDR);
.last_section :
{
  LONG(0xE015E015)
} > RAM
